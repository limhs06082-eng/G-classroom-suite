...
| ④ | **연수 현장의 진입 장벽.** fork만으로는 Firebase가 붙지 않는다(콘솔 프로젝트 생성 → 웹앱 등록 → config 복사, 1인당 15~25분) | **localStorage가 기본, Firebase는 선택.** 아래 §12.3 방식으로 붙인다 |

### 12.3 Firebase 설정값을 두는 위치 — 코드 파일 (확정)

**결정: Vercel 환경변수가 아니라 저장소 안의 설정 파일에 둔다.**

근거:
- Firebase 웹 설정(apiKey·projectId·appId 등)은 **비밀이 아니다.** Vite의 `VITE_*`
  환경변수는 빌드 시점에 번들로 박히므로, 환경변수에 넣어도 배포된 JS에서 그대로 읽힌다.
  즉 환경변수로 얻는 보안 이득이 **0**이다. 실제 보호는 Firestore 보안 규칙이 한다.
- 원본 `G-call-teachers`도 `firebase-applet-config.json`을 저장소에 커밋해 두었다.
  AI 스튜디오의 기본 방식이고 잘못된 것이 아니다.
- **동선이 결정적이다.** 수강생은 AI 스튜디오에서 코드를 고치는 중에 Firebase를 붙인다.
  환경변수 방식은 AI 스튜디오 → Vercel 대시보드로 맥락을 옮겼다가 재배포까지 눌러야 하고,
  값에 오타가 나도 화면에 아무 표시가 없다. 연수에서 이탈이 가장 많이 나는 구간이다.

따라야 할 것:
- 설정 파일은 **한 개**로 두고 위치를 고정한다. Firebase를 붙이는 주체가 사람이 아니라
  **AI 스튜디오 에이전트**이므로, 손댈 지점이 흩어져 있으면 엉뚱한 곳을 고친다.
- 환경변수 병행 지원은 하지 않는다. 분기가 늘면 에이전트가 건드릴 지점도 늘어난다.
- 필수 환경변수는 만들지 않는다. fork 직후 설정 없이 배포되고 localStorage 모드로 동작해야 한다.

공개 저장소일 때의 잔여 위험과 대응(12단계 안내 문서에 포함):
- 데이터는 보안 규칙이 막지만, 무료 할당량을 소진시키는 요청 공격은 막지 못한다.
- 대응: **App Check 활성화**를 기본 안내로 하고, 대안으로 fork본을 비공개로 두는 방법을 함께 적는다.
  (fork본이 비공개여도 Vercel 배포에는 지장이 없다.)
