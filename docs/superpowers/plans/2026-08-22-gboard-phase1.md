...
- **기능 논리를 고치지 않는다.** 자리 배치·당번 배정·점수 계산·과제 판정 같은 규칙은 건드리지 않는다. `src/features/` 아래에서 허용되는 변경은 딱 둘 — (1) 전자칠판을 여는 방식(`target="_blank"` → `openBoard()`, Task 6), (2) 홈의 형성평가 카드를 설치형에서 안내 카드로 바꾸는 것(Task 8). 둘 다 화면을 어떻게 여느냐에 대한 것이지 기능 규칙이 아니다.
