...
> **수업·업무 도구함(`G-teacher-toolkit`)도 쓰신다면** Firebase 프로젝트는 하나만 만들면 됩니다.
> 1·2단계와 4단계 보안 규칙을 한 번만 하고, 같은 설정값을 양쪽 저장소에 넣으세요.
> 문서 경로가 `teachers/{uid}/suite/data`와 `teachers/{uid}/toolkit/data`로 나뉘어
> 자료가 섞이지 않고, 4단계 규칙의 `{document=**}`가 둘 다 덮습니다.
