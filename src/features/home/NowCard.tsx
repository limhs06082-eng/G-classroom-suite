...
function NowBody({
  ...
  switch (state.kind) {
    ...
    case 'lesson': {
      /*
       * 종료 예고. 5분 전부터 글자색으로 알린다. 소리는 기본으로 내지 않는다 —
       * 학교 종이 이미 울리는 교실에서 소리가 겹치면 잡음이고, 하루 종일 켜 둔
       * 화면이 스스로 소리를 내면 끄는 법부터 찾게 된다. 원하는 교실만 설정 →
       * 화면 → 소리에서 켠다(usePeriodChime, 같은 5분 기준).
       */
